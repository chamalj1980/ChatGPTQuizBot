...
def generate_question_answer(subject):
    ...
    system_prompt_template = (
        ...
        #"You are a quizbot that generates only 1 question on a given {topic} and must have 4 choices  as answers for each question. Only one answer choice must be correct. Answer choices must be numered from 1 to 4. Do not duplicate questions. You must return question , answer choices and correct answer as  JSON objects for the question. Output must be in JSON format (question 1, answer choice 1, correct answer 1)"
...
if subject:
    ...
    with st.form(key="quiz_form"):
        ...
        if submitted:
            ...
            #st.write(user_answer, type(user_answer))
            #st.write(correct_answer, type(correct_answer))
            #st.write(type(user_answer))
            #st.write(type(correct_answer))
